(ns lallafa.core-test
  (:use clojure.test)
  (:require
   [lallafa.core :as csv]
   [lallafa.utils :as utils]))

(set! *warn-on-reflection* true)

(defn sample-csv []
  (utils/tmp-file-with "sample" ".csv"
...
(defn sample-psv []
  (utils/tmp-file-with "sample" ".psv"
...
(def sample-field-reader-opts
  {:key-fn :id
   ...
   0 {:label :id :reader utils/parse-int}
   1 {:label :left :reader utils/parse-int}
   2 {:label :right :reader utils/parse-int}
...
(deftest parsing-csv-into-nested-maps
  (let [result
        ...
        coll-result
        (csv/rows->coll result sample-field-reader-opts)

        id-1-p-result
        (csv/rows->coll result (assoc sample-field-reader-opts :pred-fn #(= (:id %) 1)))]
